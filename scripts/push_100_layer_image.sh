...
#   Each layer gets unique content (UUID per layer), like quay-performance-scripts image_load.sh.
#

set -e

# Generate a UUID (macOS: uuidgen, Linux: /proc/sys/kernel/random/uuid or uuidgen)
gen_uuid() {
  if command -v uuidgen &>/dev/null; then
    ...
    echo "Error: need uuidgen or /proc/sys/kernel/random/uuid for RANDOM_LAYERS" >&2
    exit 1
  fi
}
...
echo "Config: ${NUM_LAYERS} layers (random UUID per layer), base ${BASE_IMAGE}, repo ${QUAY_HOST}/${REPO_PATH}, images ${START}..${END}"
echo "Engine: ${ENGINE}"
echo ""
...
for idx in $(seq "$START" "$END"); do
  ...
  # Generate Dockerfile with unique content per layer (UUID), like quay-performance-scripts image_load.sh
  {
    echo "FROM ${BASE_IMAGE}"
    for i in $(seq 1 "$NUM_LAYERS"); do
      rand=$(gen_uuid)
      echo "RUN echo \"This is layer $i for image $idx with UUID $rand\" > /layer_${i}.txt"
    done
    echo "CMD [\"sh\"]"
